...
if ($r_norm && $r_norm->num_rows > 0)
    $need_norm = (int) ($r_norm->fetch_assoc()['cnt'] ?? 0);
if ($need_norm > 0) {
    $conn->query("
...
while ($row = $result->fetch_assoc()) {
    $bookings_data[] = $row;
    if (in_array($row['status'], ['booked', 'rescheduled']))
...
if (!empty($booking_ids)) {
    ...
    foreach ($bookings_data as $b) {
        $bid = (int) $b['id'];
        if ($b['status'] !== 'booked' || !isset($needs_by_booking[$bid]))
            continue;
...
<div class="container-fluid">
    ...
    <style>
        ...
        #bookingTable_wrapper .dropdown-menu {
            z-index: 2000;
            max-height: 260px;
            overflow: auto;
        }
        ...
        <div class="card-body">
            <form method="GET" class="row g-3 align-items-end">
                ...
                <div class="col-xl-2 col-lg-6 col-md-6">
                    ...
                    <div class="segmented-control">
                        <input type="radio" class="btn-check" name="tujuan" id="filter_tujuan_all" value=""
                            <?= $filter_tujuan === '' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_tujuan_all">Semua</label>

                        <input type="radio" class="btn-check" name="tujuan" id="filter_tujuan_clinic" value="clinic"
                            <?= $filter_tujuan === 'clinic' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_tujuan_clinic">Klinik</label>

                        <input type="radio" class="btn-check" name="tujuan" id="filter_tujuan_hc" value="hc"
                            <?= $filter_tujuan === 'hc' ? 'checked' : '' ?>>
                ...
                <div class="col-xl-4 col-lg-6 col-md-6">
                    ...
                    <div class="segmented-control">
                        <input type="radio" class="btn-check" name="status" id="filter_status_all" value=""
                            <?= $filter_status === '' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_status_all">Semua</label>

                        <input type="radio" class="btn-check" name="status" id="filter_status_booked" value="booked"
                            <?= $filter_status === 'booked' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_status_booked">Booked</label>

                        <input type="radio" class="btn-check" name="status" id="filter_status_completed"
                            value="completed" <?= $filter_status === 'completed' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_status_completed">Completed</label>

                        <input type="radio" class="btn-check" name="status" id="filter_status_rescheduled"
                            value="rescheduled" <?= $filter_status === 'rescheduled' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_status_rescheduled">Reschedule</label>

                        <input type="radio" class="btn-check" name="status" id="filter_status_cancelled"
                            value="cancelled" <?= $filter_status === 'cancelled' ? 'checked' : '' ?>>
                ...
                <div class="col-xl-3 col-lg-6 col-md-6">
                    ...
                    <div class="segmented-control">
                        <input type="radio" class="btn-check" name="tipe" id="filter_tipe_all" value=""
                            <?= $filter_tipe === '' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_tipe_all">Semua</label>

                        <input type="radio" class="btn-check" name="tipe" id="filter_tipe_keep" value="keep"
                            <?= $filter_tipe === 'keep' ? 'checked' : '' ?>>
                        <label class="btn-segmented px-1" for="filter_tipe_keep">Keep</label>

                        <input type="radio" class="btn-check" name="tipe" id="filter_tipe_fixed" value="fixed"
                            <?= $filter_tipe === 'fixed' ? 'checked' : '' ?>>
            ...
            <?php if ($has_filters): ?>
                <div class="mt-3 d-flex flex-wrap gap-2">
                    <?php if ($filter_today): ?><span class="badge rounded-pill booking-chip bg-light text-dark">Hari
                            ini</span><?php endif; ?>
                    <?php if ($filter_tujuan === 'clinic'): ?><span
                            class="badge rounded-pill booking-chip bg-light text-dark">Tujuan: Klinik</span><?php endif; ?>
                    <?php if ($filter_tujuan === 'hc'): ?><span
                            class="badge rounded-pill booking-chip bg-light text-dark">Tujuan: HC</span><?php endif; ?>
                    <?php if ($filter_status !== ''): ?><span
                            class="badge rounded-pill booking-chip bg-light text-dark">Status:
                            <?= htmlspecialchars($filter_status) ?></span><?php endif; ?>
                    <?php if ($filter_tipe !== ''): ?><span class="badge rounded-pill booking-chip bg-light text-dark">Tipe:
                            <?= htmlspecialchars($filter_tipe) ?></span><?php endif; ?>
                    <?php if ($filter_fu === '1'): ?><span
                            class="badge rounded-pill booking-chip bg-light text-dark">Follow-up: Ya</span><?php endif; ?>
                    <?php if ($filter_q !== ''): ?><span class="badge rounded-pill booking-chip bg-primary text-white">Cari:
                            <?= htmlspecialchars($filter_q) ?></span><?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
    </div>
                    ...
                    <form method="GET" action="index.php">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" name="q" class="form-control border-start-0 ps-0"
                                placeholder="Cari Nama / No Booking / Input By"
                                value="<?= htmlspecialchars($filter_q) ?>" style="border-radius: 0 8px 8px 0;">
                        </div>
                    </form>
                </div>
            </div>
                        ...
                        <?php foreach ($bookings_data as $row): ?>
                            <tr>
                                <td>
                                    ...
                                    <div class="mb-1">
                                        <?php if ($jf === 1): ?>
                                            <span class="badge bg-success x-small"><i
                                                    class="fas fa-check me-1"></i>Jotform</span>
                                        <?php else: ?>
                                            <span class="badge bg-danger x-small"><i
                                                    class="fas fa-times me-1"></i>Jotform</span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="x-small fw-bold text-primary mt-1 text-nowrap">
                                ...
                                <td>
                                    ...
                                    <div class="mb-1">
                                        ...
                                        <?php else: ?>
                                            <span class="badge bg-primary x-small"><i
                                                    class="fas fa-hospital me-1"></i>Klinik</span>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <td data-order="<?= $row['created_at'] ?>">
                                    <div class="booking-muted" style="font-size: 0.8rem;">
                                        <div class="fw-bold text-dark"
                                            ...
                                            <i
                                                class="fas fa-user-edit me-1"></i><?= htmlspecialchars($row['creator_name'] ?? $row['cs_name'] ?? '-') ?>
                                        </div>
                                        <div class="text-muted small mt-1"><i
                                                class="fas fa-clock me-1"></i><?= date('d/m/y H:i', strtotime($row['created_at'])) ?>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($row['approval_reason'])): ?>
                                        <div class="x-small text-muted mt-1"
                                            title="<?= htmlspecialchars($row['approval_reason']) ?>">
                                            <i class="fas fa-info-circle"></i>
                                            <?= substr(htmlspecialchars($row['approval_reason']), 0, 20) ?>        <?= strlen($row['approval_reason']) > 20 ? '...' : '' ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td class="text-end" style="vertical-align: top; width: 120px;">
                                    ...
                                    <button type="button"
                                        class="btn btn-sm btn-outline-primary btn-view-eye px-3 rounded-pill mb-2"
                                        title="Lihat Detail" onclick="return openBookingDetail(<?= (int) $row['id'] ?>);">
                                    ...
                                    <?php if (in_array($row['status'], ['booked', 'rescheduled', 'pending_edit', 'pending_delete', 'rejected'])): ?>
                                        <button type="button" class="btn btn-sm btn-primary px-3 rounded-pill btn-aksi-toggle"
                                            onclick="toggleActionDrawer(this)">
                                        ...
                                        <div class="action-drawer">
                                            <?php if (in_array($row['status'], ['booked', 'rescheduled'])): ?>
                                                ...
                                                <?php if ($is_admin_klinik || $is_spv_klinik || $is_super_admin): ?>
                                                    ...
                                                    <?php endif; ?>
                                                    <button type="button" class="btn-drawer-icon text-warning" title="Reschedule"
                                                        onclick="openRescheduleModal(<?= (int) $row['id'] ?>, '<?= htmlspecialchars($row['nomor_booking'], ENT_QUOTES) ?>', '<?= $row['tanggal_pemeriksaan'] ?>');">
                                                        <i class="fas fa-calendar-alt"></i>
                                                    </button>
                                                    <button type="button" class="btn-drawer-icon text-success" title="Done"
                                                        onclick="return openCompletionModal(<?= (int) $row['id'] ?>, '<?= htmlspecialchars($row['nomor_booking'], ENT_QUOTES) ?>');">
                                            ...
                                            <?php elseif (strpos($row['status'], 'pending') !== false): ?>
                                                <?php if ($is_super_admin): ?>
                                                    <button type="button" class="btn-drawer-icon text-success" title="Approve"
                                                        onclick="approveBookingRequest(<?= (int) $row['id'] ?>, '<?= $row['status'] ?>')">
                                                        <i class="fas fa-check"></i>
                                                    </button>
                                                    <button type="button" class="btn-drawer-icon text-danger" title="Reject"
                                                        onclick="rejectBookingRequest(<?= (int) $row['id'] ?>)">
                                                        <i class="fas fa-times"></i>
                                                    </button>
                                                <?php endif; ?>
                                            <?php endif; ?>
    ...
    <!-- Modal Booking Baru -->
    <div class="modal fade" id="modalBookingBaru" tabindex="-1" aria-labelledby="modalBookingBaruLabel"
        aria-hidden="true">
                ...
                <form id="formBooking" method="POST">
                    ...
                    <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                        <div id="bookingStockWarning" class="alert alert-warning py-2 small mb-3 d-none">
                            <i class="fas fa-exclamation-triangle me-1"></i> <strong>Peringatan:</strong> Core kosong:
                            proses tetap lanjut sesuai kebijakan, mohon follow up restock.
                        </div>
                        <div class="alert alert-info py-2 small mb-3">
                            <i class="fas fa-info-circle me-1"></i> Isi data utama, lalu pilih pemeriksaan. Qty
                            pemeriksaan otomatis mengikuti Pax.
                        </div>
                        <div class="row g-3">
                            <div class="col-12">
                                <div class="card border-0 shadow-sm mb-3">
                                    <div
                                        ...
                                        <div class="fw-bold"><i class="fas fa-tag me-2 text-primary-custom"></i>Info
                                            Booking</div>
                                        <div class="small text-muted">
                                            <i class="fas fa-user-tie me-1"></i>CS: <span
                                                class="fw-semibold text-primary"><?= htmlspecialchars($_SESSION['nama_lengkap'] ?? '') ?></span>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                                <label class="form-label fw-semibold">Status Booking <span
                                                        class="text-danger">*</span></label>
                                                <div class="segmented-control">
                                                    <input type="radio" class="btn-check" name="status_booking"
                                                        id="status_clinic" value="Reserved - Clinic" checked>
                                                    <label class="btn-segmented" for="status_clinic">Clinic</label>

                                                    <input type="radio" class="btn-check" name="status_booking"
                                                        id="status_hc" value="Reserved - HC">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <input type="number" name="jumlah_pax" id="jumlah_pax"
                                                    class="form-control bg-light" min="1" max="10" value="1" readonly
                                                    required>
                                            </div>
                                            <div class="col-md-3" id="order_id_container_modal" style="display: none;">
                                                <label class="form-label fw-semibold">Order ID</label>
                                                <input type="text" name="order_id" class="form-control"
                                                    placeholder="Opsional (Contoh: B12345)">
                                            </div>

                                            <div class="col-md-4">
                                                <label class="form-label fw-semibold">Jadwal Pemeriksaan <span
                                                        class="text-danger">*</span></label>
                                                <div class="input-group">
                                                    <input type="date" name="tanggal" id="booking_tanggal"
                                                        class="form-control" value="<?= date('Y-m-d') ?>"
                                                        min="<?= date('Y-m-d') ?>" required title="Tanggal">
                                                    <input type="time" name="jam_layanan" id="booking_jam"
                                                        class="form-control" title="Jam Layanan" required>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <label class="form-label fw-semibold">Tipe (Fixed/Keep) <span
                                                        class="text-danger">*</span></label>
                                                <div class="segmented-control">
                                                    <input type="radio" class="btn-check" name="booking_type"
                                                        id="type_keep" value="keep" checked>
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <div class="segmented-control">
                                                    <input type="radio" class="btn-check" name="jotform_submitted"
                                                        id="jotform_no" value="0" checked>
                                                    <label class="btn-segmented" for="jotform_no">Belum</label>

                                                    <input type="radio" class="btn-check" name="jotform_submitted"
                                                        id="jotform_yes" value="1">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <div class="fw-bold text-primary-custom"><i class="fas fa-users me-2"></i>Identitas
                                        Pasien</div>
                ...
                <div class="modal-header pb-0 border-0" style="background-color: #204EAB !important;">
                    <div class="d-flex flex-column w-100">
                        ...
                        <div class="d-flex justify-content-between align-items-end px-2"
                            style="margin-bottom: -1px; padding-top: 8px;">
                            <ul class="nav nav-tabs border-0" id="bookingDetailTabs" role="tablist" style="gap: 4px;">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active py-2 px-4 fw-bold border-0 rounded-top" id="info-tab"
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link py-2 px-4 fw-bold border-0 rounded-top" id="history-tab"
                                        data-bs-toggle="tab" data-bs-target="#history-pane" type="button" role="tab"
                                        style="font-size: 0.85rem; transition: all 0.2s;"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" id="adjustAdditionalPax" class="form-control bg-light" value="1" readonly
                            required>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="rescheduleReason" class="form-control" rows="3"
                            placeholder="Contoh: Kondisi pasien tidak memungkinkan (tensi tinggi), permintaan pasien, dsb."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
